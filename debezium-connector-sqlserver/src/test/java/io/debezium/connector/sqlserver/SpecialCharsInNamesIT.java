...
public class SpecialCharsInNamesIT extends AbstractConnectorTest {
    ...
    public void shouldParseWhitespaceChars() throws Exception {
        ...
        TestHelper.forEachDatabase(databaseName -> {
            assertThat(actualRecords.recordsForTopic(TestHelper.topicName(databaseName, "UAT_WAG_CZ_Fixed_Asset"))).hasSize(1);
            assertThat(actualRecords.recordsForTopic(TestHelper.topicName(databaseName, "UAT_WAG_CZ_Fixed_Prop"))).hasSize(1);
    ...
    public void shouldParseSpecialChars() throws Exception {
        ...
        TestHelper.forEachDatabase(databaseName -> {
            assertThat(snapshotRecords.recordsForTopic(TestHelper.topicName(databaseName, "UAT_WAG_CZ_Fixed_Asset"))).hasSize(1);
        ...
        TestHelper.forEachDatabase(databaseName -> {
            connection.execute("USE " + databaseName);
            assertThat(streamingRecords.recordsForTopic(TestHelper.topicName(databaseName, "UAT_WAG_CZ_Fixed_Asset"))).hasSize(1);
            ...
            assertThat(records.recordsForTopic(TestHelper.topicName(databaseName, "UAT_WAG_CZ_Fixed_Asset_Two"))).hasSize(1);
    ...
    public void shouldHandleSpecialCharactersInDatabaseNames() throws Exception {
        ...
        final Configuration config = TestHelper.defaultConfig()
                .with(SqlServerConnectorConfig.SNAPSHOT_MODE, SnapshotMode.INITIAL)
                .with(SqlServerConnectorConfig.DATABASE_NAMES, databaseName)
        ...
        List<SourceRecord> records = recordsByTopic.recordsForTopic(TestHelper.topicName(databaseName, "tablea"));
